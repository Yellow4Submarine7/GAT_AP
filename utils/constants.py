...
#换成Amazon的数据,这里以项目-用户对（评分）(0-10260)来划分训练测试及验证部分


'''我的任务中，Amazon_Musical_Instruments 数据集共有10261条交易记录，它们由2329个用户和项目交互产生。
那么在训练过程中，图节点的表征获取部分应该不对节点类型做区分，loss部分应该使用项目-用户对的评分来生成
每个epoch中实际将数据放到GAT跑一遍，然后再将数据根据索引拿出分析，获得loss用于后向传播
所以索引只用于产生loss，这样的话在我的任务里就是需要将node indices全部改成item indices 和user indices
注意csv和txt只能存string，其他例如数组tensor等一律用专门的存储方式如np.save或torch.save存储
'''
item_indices_list = np.load('data/item_indices.npy').tolist()
user_indices_list = np.load('data/user_indices.npy').tolist()
AMAZON_TRAIN_RANGE = [0,6000]
AMAZON_TRAIN_USER_LIST = item_indices_list[0:6000]         #[0,6000]内可重复的数
AMAZON_TRAIN_ITEM_LIST = user_indices_list[0:6000]         #[0,6000]内可重复的数
AMAZON_VAL_RANGE = [6000, 9000]
AMAZON_VAL_USER_LIST = item_indices_list[6000:9000]            #[6000,9000]内可重复的数
AMAZON_VAL_ITEM_LIST = user_indices_list[6000:9000]            #[6000,9000]内可重复的数
AMAZON_TEST_RANGE = [9000,10260]
AMAZON_TEST_USER_LIST = item_indices_list[9000:10260]           #[9000,10260]内可重复的数
AMAZON_TEST_ITEM_LIST = user_indices_list[9000:10260]           #[9000,10260]内可重复的数
AMAZON_NUM_INPUT_FEATURES = 100 #随机，暂定
AMAZON_RATING_CLASSES = 6
